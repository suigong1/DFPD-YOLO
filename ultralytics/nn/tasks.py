...
DETECT_CLASS = (Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, Detect_Efficient, DetectAux, Detect_SEAM, Detect_MultiSEAM, 
                Detect_DyHeadWithDCNV3, Detect_DyHeadWithDCNV4, Detect_DyHead_Prune, Detect_LSCD, Detect_DTDH, Detect_LADH, Detect_LSCSBD, Detect_LSDECD)
V10_DETECT_CLASS = (v10Detect, Detect_NMSFree, v10Detect_LSCD, v10Detect_SEAM, v10Detect_MultiSEAM)
SEGMENT_CLASS = (Segment, Segment_Efficient, Segment_LSCD, Segment_DTDH, Segment_LADH, Segment_LSCSBD, Segment_LSDECD)
POSE_CLASS = (Pose, Pose_LSCD, Pose_DTDH, Pose_LADH, Pose_LSCSBD, Pose_LSDECD)
OBB_CLASS = (OBB, OBB_LSCD, OBB_DTDH, OBB_LADH, OBB_LSCSBD, OBB_LSDECD)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in SEGMENT_CLASS:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
                if m in (Segment_LSCD, Segment_DTDH, Segment_LSCSBD, Segment_LSDECD):
                    args[3] = make_divisible(min(args[3], max_channels) * width, 8)
            if m in (Detect_LSCD, Detect_DTDH, Detect_LSCSBD, Detect_LSDECD, v10Detect_LSCD):
                args[1] = make_divisible(min(args[1], max_channels) * width, 8)
            if m in (Pose_LSCD, Pose_DTDH, Pose_LSCSBD, Pose_LSDECD, OBB_LSCD, OBB_DTDH, OBB_LSCSBD, OBB_LSDECD):
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
